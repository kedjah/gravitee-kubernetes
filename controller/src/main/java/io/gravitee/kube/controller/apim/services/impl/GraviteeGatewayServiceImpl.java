...
import io.gravitee.kube.controller.apim.services.GraviteeGatewayService;
...
import static io.gravitee.kube.controller.apim.utils.ControllerDigestHelper.computeGenericConfigHashCode;
...
public class GraviteeGatewayServiceImpl
    ...
    protected WatchActionContext<GraviteeGateway> computeBackendConfigHashCode(WatchActionContext<GraviteeGateway> context) {
        ...
        GraviteeGatewaySpec spec = context.getResource().getSpec();
        String hashCode = computeGenericConfigHashCode(spec.getDefaultBackendConfigurations());
    ...
    private boolean hasChanged(WatchActionContext<GraviteeGateway> context, GraviteeGatewayStatus status, Map<String, String> newHashCodes) {
        return !Maps.difference(newHashCodes, status.getHashCodes().getPlugins()).areEqual()
