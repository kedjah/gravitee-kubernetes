...
import io.gravitee.kube.controller.apim.services.GraviteeServicesService;
import io.gravitee.kube.controller.apim.services.impl.WatchActionContext;
import io.gravitee.kube.controller.apim.watcher.GraviteeServiceWatcher;
...
public class GraviteeServicesManager extends AbstractLifecycleComponent<GraviteeServicesManager> implements Publisher<WatchActionContext> {
    ...
    public void subscribe(Subscriber<? super WatchActionContext> subscriber) {
        this.serviceWatcher = this.graviteeServices.getCrdClient()
                .watch(new GraviteeServiceWatcher(
                        subscriber,
                        pluginsService,
                        gatewayService,
                        graviteeServices));
        ...
        Flowable.fromPublisher(this)
                ...
                .doOnError((error) -> {
                    if (error instanceof PipelineException) {
                        ...
                        LOGGER.error("Process Action on GraviteeServices fails on resource '{}'",
                                ((PipelineException) error).getContext().getResource(), error);
                    } else {
